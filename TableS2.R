...
# run Model 1 for 100 replications
# using different choices of kernel functions
# this could take time
# parallelizing it is recommended
for (iter in 1:100) {
    # Build the command
    cmd = paste("Rscript run_ModelKernel.R", iter)
    cat("Running:", cmd, "\n")
    # Run the command
    system(cmd)
}

# summarize results into Table S2
...
Methods = c("GPAMgg","GPAMlg","GPAMgl","GPAMll")
...
res_xtable = lapply(res_df, function(x){
    ...
    ##xtable(xx)
    # xxx = xxx[is.element(xxx$Method, method_sel),]
    rownames(xxx)=NULL
    xxx
})
...
print(res_xtable, file=paste(folder_path,"TableS2.txt",sep=""),include.rownames=FALSE)
