...
# run Model T for 100 replications for each T in {1,2,3}
# this could take time
# parallelizing it is recommended
for (iter in 1:100) {
    for(Tmax in 1:3){
        # Build the command
        cmd = paste("Rscript run_ModelT.R", iter, Tmax)
        cat("Running:", cmd, "\n")
        # Run the command
        system(cmd)
    }
...
# summarize results into Table S1
...
settings = c("ModelT1","ModelT2","ModelT3")
p0 = 4
#p = 100
...
for(setting in settings){
    ...
    for(ffs in 1:100){
        ...
        if(file.exists(filename)){
            ...
            beta_norm = lapply(res_list, function(x){x$beta[-1]})
...
res_xtable = lapply(seq_along(res_df), function(TT){
    x = res_df[[TT]]
    xx = t(x)
...
print(res_xtable, file=paste(folder_path,"TableS1.txt",sep=""),include.rownames=FALSE)
