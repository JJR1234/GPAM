...
#### The ICU EHR dataset used in the paper can be downloaded from the website: https://physionet.org/content/challenge-2012/1.0.0/. In particular, the following files from the website are used for analysis: Outcomes-a.txt, Outcomes-b.txt, Outcomes-c.txt, set-a.tar.gz, set-b.tar.gz, and set-c.tar.gz. After these files have been downloaded and all *.tar.gz files have been unzipped, the following codes can be run to reproduce the corresponding results.
