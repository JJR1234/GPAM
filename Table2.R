...
# run Model 2 for 100 replications
# this could take time
# parallelizing it is recommended
for (iter in 1:100) {
    # Build the command
    cmd = paste("Rscript run_Model2.R", iter)
    cat("Running:", cmd, "\n")
    # Run the command
    system(cmd)
}

# summarize results into Table 2
...
print(res_xtable, file=paste(folder_path,"Table2.txt",sep=""),include.rownames=FALSE)
